/*
This code initializes and operates on XIAO BLE SENSE with sensors and Bluetooth Low Energy (BLE) communication. 
It configures an Inertial Measurement Unit (IMU) for reading accelerometer and gyroscope data, processes these sensor inputs, 
and uses a neural network for classification. The results are sent via BLE and displayed on a screen, with the system 
responding to specific commands like "hello" and "bye" by controlling a motor and updating the display.
*/
    ...
    if (previous_label == nullptr || strcmp(previous_label, max_label) != 0) {
      ...
      if (isActive) {
        if (strcmp(max_label, "light") == 0) {
          ...
          updatevalue("display");
          Display.DrawString_EN(30, 125, "Tem:22 Hum:46%", &Font16, WHITE, BLACK);
